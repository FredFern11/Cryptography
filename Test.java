...
public class Test {
    public static void testDecryption(int n) {
        ...
        for (int i = 1; i < n+1; i++) {
            ...
            key = AES.randStr(16);
            encoder = new Encoder(key.getBytes());
            decoder = encoder.genDecoder();
            recovered = new String(decoder.decrypt(encoder.encrypt(message.getBytes())));
...
//            char[] charArr = new char[random.nextInt(100)];
            ...
            String myAes = encoder.toBase64(encoder.encrypt(text.getBytes(), IV.getIV()));
