...
//                state[i][j] = substitute(state[i][j], table);
     ...
     */
    public Matrix shiftRows(Matrix state, boolean enc) {
        for (int i = 1; i < state.getLength(); i++) {
            byte[] memory = new byte[4];
            for (int j = 0; j < memory.length; j++)
                memory[j] = state.get(j,i);
            for (int j = 0; j < state.getLength(); j++)
                state.set(j, i, memory[(j + (enc? 1 : -1 ) * i + 4) % 4]);;
        }
        return state;
    }
     ...
     * @return new state
     */
    public Matrix mixColumns(Matrix state, byte[][] matrix) {
        return state.multi(new Matrix(matrix));
    }
}
